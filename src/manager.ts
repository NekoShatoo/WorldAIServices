...
async function buildSessionToken(env: Env) {
	const payloadJson = JSON.stringify({ exp: Date.now() + SESSION_TTL_SECONDS * 1000 });
...
export async function handleManagerApi(request: Request, env: Env, ctx: ExecutionContext, url: URL): Promise<Response> {
	...
	if (path === '/login' && request.method === 'POST') {
		...
		if (!password) return jsonResponse({ status: 'error', result: 'Password required' }, 400);
	...
	if (path === '/stats' && request.method === 'GET') return jsonResponse({ status: 'ok', result: await loadTranslationStatsSummary(env) });
	...
	if (path === '/simulate' && request.method === 'POST') {
		...
		if (!lang) return jsonResponse({ status: 'error', result: 'lang を指定してください。' }, 400);
